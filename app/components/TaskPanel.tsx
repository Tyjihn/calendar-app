...
import { ClipboardList, ChevronDown, Trash2, Clock, Sparkles } from "lucide-react";
...
export default function TaskPanel() {
    ...
    return (
        ...
        <div className="flex flex-col h-full text-black gap-4">
            {/* 3 Tab Buttons */}
            <div className="flex bg-gray-100 h-10 p-1 rounded-lg gap-1">
                {tabs.map((t) => (
                    <button key={t.id} type="button" onClick={() => setTab(t.id)}
                        className={`flex-1 rounded-md px-3 font-sans transition cursor-pointer ${
                            tab === t.id ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:font-[550]'
                        }`}>
                        {t.label}
                    </button>
                ))}
